...
public class SurvivorBird extends ApplicationAdapter {
	...
	public void create () {
		...
		beeX = Gdx.graphics.getWidth() / 2 - bee.getHeight() / 2;
		beeY = Gdx.graphics.getHeight() / 3;
	...
	public void render () {
		...
		if (gameState ==1) {
			...
			if (enemyX[scoredEnemy] < Gdx.graphics.getWidth() / 2 - bee.getHeight() / 2) {
				score++;
			...
			for (int i = 0; i < numberOfEnemies; i++) {
				...
				batch.draw(enemy2,enemyX[i],Gdx.graphics.getHeight()/2 + enemyOffSet3[i],Gdx.graphics.getWidth() / 15,Gdx.graphics.getHeight() / 10);
		...
		batch.draw(bee,beeX, beeY, Gdx.graphics.getWidth() / 15,Gdx.graphics.getHeight() / 10);

		font.draw(batch,String.valueOf(score),100,200);

		batch.end();

		birdCircle.set(beeX +Gdx.graphics.getWidth() / 30 ,beeY + Gdx.graphics.getHeight() / 20,Gdx.graphics.getWidth() / 30);
